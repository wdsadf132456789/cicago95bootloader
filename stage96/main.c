...
void stage96_entry(void) {
    kf(); clr(0);
    txt((COLS-14)/2,0,"SQL Demo (Stage 96)",7);
    const char *data[]={"1|Alice|25|NYC","2|Bob|31|SF","3|Carol|22|LA","4|Dave|38|CHI","5|Eve|29|SEA"};
    for(int f=0;f<120;f++) {
        clr(0);
        txt((COLS-14)/2,0,"SQL Demo (Stage 96)",7);
        txt(2,2,"SELECT users.name, orders.total",7+2);
        txt(2,3,"FROM users",7+2);
        txt(2,4,"JOIN orders ON users.id = orders.user_id",7+4);
        txt(2,5,"WHERE users.age > 25",7);
        txt(2,6,"ORDER BY orders.total DESC;",7);
        txt(2,8,"+----+-------+-----+-------+",7+1);
        txt(2,9,"| id | name  | age | city  |",7+3);
        txt(2,10,"+----+-------+-----+-------+",7+1);
        for(int i=0;i<5;i++) {
            int hi=(f%5)==i;
            txt(2,11+i,data[i],hi?7+6:7);
        }
        txt(2,16,"+----+-------+-----+-------+",7+1);
