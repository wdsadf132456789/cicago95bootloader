...
void stage35_entry(void) {
    kf(); clr(0);
    txt((COLS-16)/2,0,"Rust Demo (Stage 35)",6);
    for(int f=0;f<120;f++) {
        clr(0);
        txt((COLS-16)/2,0,"Rust Demo (Stage 35)",6);
        txt(2,2,"let v = vec![1,2,3,4,5];",6+2);
        txt(2,4,"let doubled: Vec<i32> =",7);
        txt(2,5,"    v.iter().map(|x| x * 2).collect();",7);
        for(int i=0;i<5;i++) {
            int val=(i+1)*(1+f%6);
            pn(6+i*5,8,val,6+(i%7));
            px(10+i*5,8,',',7);
        }
        txt(2,10,"let owned = v.clone(); // ownership",6+4);
        txt(2,12,"match doubled[0] {",6+2);
        txt(2,13,"  2 => println!(\"first is 2\"),",7);
        txt(2,14,"  _ => (),",7);
        txt(2,15,"}",6+2);
        txt(2,17,"fn greet(name: &str) -> String {",6+3);
